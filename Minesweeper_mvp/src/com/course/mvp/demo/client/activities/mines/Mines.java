...
public class Mines {
	...
	public int getSize(){
		return  view.getHeaderPanel().getOffsetHeight();
	}
	...
	class touch implements TouchStartHandler {
		...
		public void onTouchStart(TouchStartEvent event) {
			...
			y = event.getNativeEvent().getClientY()-52;
			cCol = x / CELL_SIZE_W;
			cRow = y / CELL_SIZE_H;
	...
	class Myclick implements ClickHandler {
		...
		public void onClick(ClickEvent event) {
			...
			if (event.getNativeButton() == NativeEvent.BUTTON_LEFT
					...
					|| event.getNativeButton() == NativeEvent.BUTTON_MIDDLE) {
				x = event.getClientX();
			...
			if ((x < (N_COLS) * CELL_SIZE_W)
					&& (y < (N_ROWS) * CELL_SIZE_H)) {
